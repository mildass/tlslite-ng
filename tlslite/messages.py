...
class Heartbeat(object):
    ...
    def checkResponse(self, callback=None):
        """
        Checks heartbeat response and calls callback function.

        If this method is called on heartbeat message, that is not at
        right format, then we don't call callback.
        """
        if self.message_type != HeartbeatMessageType.heartbeat_response:
            return
